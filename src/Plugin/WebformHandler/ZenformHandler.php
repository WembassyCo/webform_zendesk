 ...
 *   label = @Translation("Create a Zendesk Issue"),
 *   category = @Translation("Notification"),
 *   description = @Translation("Create a Zendesk Issue from Webform Submissions."),
 *   cardinality = \Drupal\webform\Plugin\WebformHandlerInterface::CARDINALITY_UNLIMITED,
 *   results = \Drupal\webform\Plugin\WebformHandlerInterface::RESULTS_PROCESSED,
 *   submission = \Drupal\webform\Plugin\WebformHandlerInterface::SUBMISSION_REQUIRED,
 * )
 */
class ZenformHandler extends WebformHandlerBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    if (empty($roles_element_options) && $this->currentUser->hasPermission('administer webform')) {
      ...
      $t_args = [
        ':href' => Url::fromRoute($route_name, [], $route_options)->toString(),
      ];
    ...
    $form['message']['body_custom_twig'] = [
      ...
      // Must set #parents because body_custom_twig is not configuration value.
      // @see \Drupal\webform\Plugin\WebformHandler\EmailWebformHandler::validateConfigurationForm
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    ...
    if ($this->configuration['states'] && in_array($state, $this->configuration['states'])) {
      $ticket_data = $this->getMessage($webform_submission);
      // Get webform submission and connect to zendesk API to create issue.
      ...
      $build = [
        ...
        '#message' => [
          'body' => is_string($ticket_data['body']) ? Markup::create($ticket_data['body']) : $ticket_data['body'],
        ] + $ticket_data,
  ...
  protected function buildElement($name, $title, $label, $required = FALSE, array $element_options, array $options_options = NULL, array $role_options = NULL, array $other_options = NULL) {
    ...
    // Set empty option.
    $email_settings_array = [
      'reply_to',
      'return_path',
      'sender_mail',
      'sender_name',
    ];
    if (in_array($name, $email_settings_array)) {
